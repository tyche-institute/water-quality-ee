...
export const PARAMETER_CARDS: ParameterCard[] = [
  {
    key: "e_coli",
    icon: "🧫",
    ruTitle: "E. coli",
    ...
    ruWhy: "Прямой маркер фекального загрязнения (норма: ≤500 КОЕ для купания, 0 для питьевой воды). При превышении риск кишечных инфекций резко растёт. В нашем корпусе (69 536 проб) e_coli > 500 практически гарантирует нарушение.",
    etWhy: "Otsene fekaalreostuse marker (norm: ≤500 PMÜ suplusvees, 0 joogivees). Ületamisel soolenakkuse risk kasvab järsult. 69 536 proovist e_coli > 500 tähendab peaaegu alati rikkumist.",
    enWhy: "Direct fecal contamination marker (norm: ≤500 CFU bathing, 0 drinking). Exceeding the threshold almost guarantees a violation in our 69,536-probe corpus."
  },
  {
    key: "enterococci",
    icon: "🧪",
    ...
    ruWhy: "Ключевой микробиологический показатель EU Bathing Water Directive 2006/7/EC (норма: ≤200 КОЕ). Устойчивее E. coli к хлорированию — важный дополнительный индикатор в морской воде.",
    etWhy: "EL suplusvee direktiivi 2006/7/EC põhinäitaja (norm: ≤200 PMÜ). Kloorimisele vastupidavam kui E. coli — oluline lisaindikaator merevees.",
    enWhy: "Key indicator per EU Bathing Water Directive 2006/7/EC (norm: ≤200 CFU). More resistant to chlorination than E. coli — important complementary marker."
  },
  {
    key: "ph",
    icon: "⚖️",
    ...
    ruWhy: "Норма: 6.0–9.0 (купание), 6.5–8.5 (бассейн). Влияет на коррозию труб, эффективность хлора и комфорт. При pH < 6.5 хлор менее эффективен; при pH > 8.5 кожа и глаза раздражаются.",
    etWhy: "Norm: 6.0–9.0 (suplus), 6.5–8.5 (bassein). Mõjutab torude korrosiooni, kloori efektiivsust ja mugavust. pH < 6.5: kloor vähem efektiivne; pH > 8.5: naha ja silmade ärritus.",
    enWhy: "Norm: 6.0–9.0 (bathing), 6.5–8.5 (pools). Affects pipe corrosion, chlorine efficacy and comfort. Low pH reduces disinfection; high pH irritates skin and eyes."
  },
  {
    key: "nitrates",
    icon: "🌾",
    ...
    ruWhy: "Норма: ≤50 мг/л (EU 2020/2184). Поступают из удобрений и сточных вод. Опасны для младенцев (метгемоглобинемия). В нашем аудите нитраты — один из «периодических» параметров: измеряются квартально, не в каждой пробе.",
    etWhy: "Norm: ≤50 mg/l (EU 2020/2184). Pärinevad väetistest ja reoveest. Ohtlik imikutele (methemoglobineemia). Meie auditi järgi mõõdetakse nitraate kvartaalselt, mitte igas proovis.",
    enWhy: "Norm: ≤50 mg/l (EU 2020/2184). From fertilizers and wastewater. Dangerous for infants (methemoglobinemia). Our audit shows nitrates are measured quarterly, not in every probe."
  },
  {
    key: "free_chlorine",
    icon: "🏊",
    ...
    etTitle: "Vaba kloor",
    enTitle: "Free chlorine",
    ruImpact: "Критичный в бассейнах",
    etImpact: "Basseinides kriitiline",
    enImpact: "Critical for pools",
    ruWhy: "Диапазон: 0.5–1.5 мг/л (Sotsiaalministri 49/2019, Lisa 4). Мало хлора → микробы; много → раздражение кожи и глаз. Наш аудит обнаружил, что прежний диапазон [0.2–0.6] был ошибочным — 288 ложных срабатываний исправлены.",
    etWhy: "Vahemik: 0.5–1.5 mg/l (Sotsiaalministri 49/2019, Lisa 4). Liiga vähe kloori → mikroobid; liiga palju → ärritus. Meie audit leidis, et varasem vahemik [0.2–0.6] oli vale — 288 valepositiivset parandatud.",
    enWhy: "Range: 0.5–1.5 mg/l (Estonian reg. 49/2019, Annex 4). Too low → pathogens; too high → irritation. Our audit found the previous [0.2–0.6] range was wrong — 288 false positives fixed."
  },
  {
    key: "turbidity",
    icon: "🌫️",
    ...
    ruWhy: "Норма: ≤4 NTU (питьевая), ≤0.5 NTU (бассейн, в 8× строже). Мутная вода скрывает патогены и снижает эффективность UV/хлора. Чем выше мутность, тем менее надёжна дезинфекция.",
    etWhy: "Norm: ≤4 NTU (joogivesi), ≤0.5 NTU (bassein, 8× rangem). Hägune vesi varjab patogeene ja vähendab UV/kloori efektiivsust.",
    enWhy: "Norm: ≤4 NTU (drinking), ≤0.5 NTU (pool — 8x stricter). Turbid water hides pathogens and reduces UV/chlorine efficacy."
  },
  {
    key: "iron",
    icon: "🔩",
    ...
    ruWhy: "Норма: ≤0.2 мг/л. Самый сильный предиктор нарушений (SHAP 1.217). Частая причина несоответствий в сетях водоснабжения Эстонии — природное железо из подземных вод.",
    etWhy: "Norm: ≤0.2 mg/l. Tugevaim rikkumiste ennustaja (SHAP 1.217). Sage mittevastavuse põhjus Eesti veevärkides — looduslik raud põhjaveest.",
    enWhy: "Norm: ≤0.2 mg/l. Strongest violation predictor (SHAP 1.217). Frequent cause of non-compliance in Estonian water networks — natural iron from groundwater."
  },
  {
    key: "coliforms",
    icon: "🔬",
    ...
    ruWhy: "Норма: 0 КОЕ/100 мл (питьевая вода, EU 2020/2184). Широкая группа бактерий, включающая E. coli. Измеряется чаще всех параметров (1 439 из 2 196 точек). Обнаружение при отсутствии E. coli указывает на недостаточную дезинфекцию.",
    etWhy: "Norm: 0 PMÜ/100 ml (joogivesi, EU 2020/2184). Lai bakterirühm, hõlmab E. coli. Mõõdetakse sagedamini kui ükski teine parameeter. Positiivne tulemus ilma E. coli'ta viitab ebapiisavale desinfitseerimisele.",
    enWhy: "Norm: 0 CFU/100ml (drinking, EU 2020/2184). Broad bacterial group including E. coli. Most frequently measured parameter. Detection without E. coli indicates insufficient disinfection."
  }
];

export function pickLocalizedParamLabel(lang: DashboardContentLang, copy: ParamCopy): string {
  if (lang === "ru") return copy.ruLabel;
  if (lang === "et") return copy.etLabel;
  return copy.enLabel;
}

export function pickLocalizedParamDesc(lang: DashboardContentLang, copy: ParamCopy): string {
  if (lang === "ru") return copy.ruDesc;
  if (lang === "et") return copy.etDesc;
  return copy.enDesc;
}
